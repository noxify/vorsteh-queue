...
export const CanarySearch = ({ tabs }: { tabs: { name: string; pattern: string }[] }) => {
  ...
  const ProviderComponent = ({ children }: { children: unknown }) =>
    useMockProvider ? (
      // @ts-expect-error known react 19 issue
      <canary-provider-mock>{children}</canary-provider-mock>
    ) : (
      // @ts-expect-error known react 19 issue
      <canary-provider-pagefind options={{ path: addBasePath("/pagefind/pagefind.js") }}>
        {children}
        {/* @ts-expect-error known react 19 issue */}
          ...
          <ProviderComponent>
            ...
            >
              ...
              <Button
                ...
                id="search-modal"
              >
                <SearchIcon className="sr:hidden h-4 w-4" />
              ...
              <canary-content slot="content">
                ...
                <canary-search slot="mode">
                  {/* @ts-expect-error known react 19 issue */}
                  <canary-filter-tabs-glob slot="head" tabs={JSON.stringify(tabs)}>
                    {/* @ts-expect-error known react 19 issue */}
