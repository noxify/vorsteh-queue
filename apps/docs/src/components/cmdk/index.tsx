...
// @ts-expect-error use default from cmdk
...
const Command = React.forwardRef<HTMLDivElement, CommandProps>((props, forwardedRef) => {
  ...
  const propsRef = useAsRef({ ...props, shouldFilter: props.shouldFilter ?? false })
  const { label, value, vimBindings = true, async = false, fetchInProgress = false, ...etc } = props
