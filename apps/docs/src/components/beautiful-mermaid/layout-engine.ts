...
import { clipEdgeToShape } from "./shape-clipping"
import { ARROW_HEAD, resolveRenderStyle } from "./styles"
...
function mermaidToElk(
  ...
  const edgesBySubgraph = new Map<
    string | null,
    { index: number; edge: (typeof graph.edges)[0] }[]
  >()
  ...
  for (const sg of graph.subgraphs) {
    elkGraph.children!.push(
      subgraphToElk(sg, graph, opts, style, edgesBySubgraph, subgraphPorts)
    )
  }
...
function subgraphToElk(
  ...
  >,
  style: ResolvedRenderStyle,
  edgesBySubgraph: Map<string | null, { index: number; edge: MermaidEdge }[]>,
  subgraphPorts: Map<
    string,
    {
      portId: string
  ...
  for (const child of sg.children) {
    elkNode.children!.push(
      subgraphToElk(child, graph, opts, style, edgesBySubgraph, subgraphPorts)
    )
  }
...
function collectEdgeSegments(
  ...
  if (elkNode.edges) {
    for (const elkEdge of elkNode.edges) {
      ...
      const edgeIndex = Number.parseInt(elkEdge.id.slice(1), 10)
      if (isNaN(edgeIndex)) {
        continue
      }
...
function adjustJunctionForGroups(
  ...
  if (isLR) {
    return crossingGroup.x - GAP
  }
  if (isRL) {
    return crossingGroup.x + crossingGroup.width + GAP
  }
  if (isBT) {
    return crossingGroup.y + crossingGroup.height + GAP
  }
...
function bundleEdgePaths(
  ...
  for (const edge of edges) {
    ...
    if (!fanOutGroups.has(edge.source)) {
      fanOutGroups.set(edge.source, [])
    }
  ...
  for (const [sourceId, group] of fanOutGroups) {
    ...
    const { style } = group[0]!
    if (group.some((e) => e.label || e.style !== style)) {
      continue
    }
  ...
  for (const edge of edges) {
    if (processed.has(edge) || edge.source === edge.target) {
      continue
    }
    if (!fanInGroups.has(edge.target)) {
      fanInGroups.set(edge.target, [])
    }
