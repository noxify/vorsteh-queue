...
export function runTests<TDatabase = unknown>(
  ...
  describe.each(ctx.testCases)(
    ...
    ({ modelName, schemaName, tableName, useDefault }) => {
      ...
      describe("flow field persistence", () => {
        it("should persist and return flowId via addJob", async () => {
          const job = await adapter.addJob({
            attempts: 0,
            childrenCompleted: 1,
            childrenCount: 3,
            failParentOnFailure: true,
            flowId: "00000000-0000-0000-0000-00000000aaaa",
            maxAttempts: 3,
            name: "flow-root",
            parentId: undefined,
            payload: { test: true },
          ...
          expect(job.flowId).toBe("00000000-0000-0000-0000-00000000aaaa")
          expect(job.childrenCount).toBe(3)
          expect(job.childrenCompleted).toBe(1)
          expect(job.failParentOnFailure).toBe(true)

          const retrieved = await adapter.getJobById(job.id)
          expect(retrieved?.flowId).toBe("00000000-0000-0000-0000-00000000aaaa")
          expect(retrieved?.childrenCount).toBe(3)
          expect(retrieved?.childrenCompleted).toBe(1)
          expect(retrieved?.failParentOnFailure).toBe(true)
        })

        it("should persist parentId for child jobs", async () => {
          const parent = await adapter.addJob({
            attempts: 0,
            childrenCount: 1,
            flowId: "00000000-0000-0000-0000-00000000bbbb",
            maxAttempts: 3,
            name: "parent-job",
            payload: {},
            priority: 2,
          ...
          const child = await adapter.addJob({
            attempts: 0,
            flowId: "00000000-0000-0000-0000-00000000bbbb",
            maxAttempts: 3,
            name: "child-job",
          ...
          expect(child.flowId).toBe("00000000-0000-0000-0000-00000000bbbb")

          const retrieved = await adapter.getJobById(child.id)
          expect(retrieved?.parentId).toBe(parent.id)
          expect(retrieved?.flowId).toBe("00000000-0000-0000-0000-00000000bbbb")
        })

        it("should persist flow fields via addJobs", async () => {
          const jobs = await adapter.addJobs([
            {
              attempts: 0,
              childrenCount: 2,
              flowId: "00000000-0000-0000-0000-00000000cccc",
              maxAttempts: 3,
              name: "batch-flow-root",
              payload: {},
              priority: 2,
              processAt: new Date(),
              progress: 0,
          ...
          expect(jobs[0]?.flowId).toBe("00000000-0000-0000-0000-00000000cccc")
          expect(jobs[0]?.childrenCount).toBe(2)
          expect(jobs[1]?.flowId).toBe("00000000-0000-0000-0000-00000000cccc")
          expect(jobs[1]?.parentId).toBe("00000000-0000-0000-0000-000000000099")
          expect(jobs[1]?.failParentOnFailure).toBe(true)

          const retrieved = await adapter.getJobById(jobs[0]?.id ?? "")
          expect(retrieved?.flowId).toBe("00000000-0000-0000-0000-00000000cccc")
          expect(retrieved?.childrenCount).toBe(2)
        })

        it("should default flow fields to null/0/false when not set", async () => {
          const job = await adapter.addJob({
            attempts: 0,
            maxAttempts: 3,
            name: "no-flow",
          ...
          const retrieved = await adapter.getJobById(job.id)
          expect(retrieved?.flowId).toBeUndefined()
          expect(retrieved?.parentId).toBeUndefined()
        })
      })

      describe("deleteFlow", () => {
        it("should delete all jobs in a flow", async () => {
          const flowId = "00000000-0000-0000-0000-00000000dd01"

          // Create a flow: root + 2 children
          const root = await adapter.addJob({
            attempts: 0,
            childrenCount: 2,
            flowId,
            maxAttempts: 3,
          ...
          const child1 = await adapter.addJob({
            attempts: 0,
            flowId,
            maxAttempts: 3,
            name: "flow-child-1",
          ...
          const child2 = await adapter.addJob({
            attempts: 0,
            flowId,
            maxAttempts: 3,
            name: "flow-child-2",
          ...
          expect(await adapter.getJobById(root.id)).toBeNull()
          expect(await adapter.getJobById(child1.id)).toBeNull()
          expect(await adapter.getJobById(child2.id)).toBeNull()
        })

        it("should not delete jobs from other flows", async () => {
          const flowA = "00000000-0000-0000-0000-00000000dd02"
          const flowB = "00000000-0000-0000-0000-00000000dd03"
          ...
          await adapter.deleteFlow(flowA)

          // Flow B should still exist
          const retrieved = await adapter.getJobById(jobB.id)
          expect(retrieved).not.toBeNull()
          expect(retrieved?.name).toBe("flow-b-root")
        })

        it("should return 0 for non-existent flow", async () => {
          const count = await adapter.deleteFlow(
            "00000000-0000-0000-0000-000000000000"
          )
          expect(count).toBe(0)
        })

        it("should not affect non-flow jobs", async () => {
          const flowId = "00000000-0000-0000-0000-00000000dd04"
          ...
          const regularJob = await adapter.addJob({
            attempts: 0,
            maxAttempts: 3,
            name: "regular-job",
            payload: {},
          ...
          const retrieved = await adapter.getJobById(regularJob.id)
          expect(retrieved).not.toBeNull()
          expect(retrieved?.name).toBe("regular-job")
        })
      })
