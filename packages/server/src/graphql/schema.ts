  ...
  fields: (t) => ({
    ...
    jobs: t.field({
      ...
      resolve: async (_parent, _args, ctx) => {
        // For simplicity, use stats to confirm queue exists and return dead jobs endpoint
        // A full implementation would add a generic query method to the adapter
        const stats = await ctx.adapter.getQueueStats()
        void stats
        return []
      },
    }),
