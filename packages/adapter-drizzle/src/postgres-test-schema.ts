...
export const customQueueJobs = customSchema.table(
  "custom_queue_jobs",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("idx_custom_queue_jobs_status_priority").on(
      table.queueName,
      table.status,
      table.priority,
      table.createdAt,
    ),
    index("idx_custom_queue_jobs_process_at").on(table.processAt),
  ],
)
